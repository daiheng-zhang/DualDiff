...
class PrepLig(object):
    def __init__(self, input_mol, mol_format):
        if mol_format == 'smi':
            ...
            self.rd_mol = Chem.MolFromSmiles(input_mol)
        elif mol_format == 'sdf': 
            self.ob_mol = next(pybel.readfile(mol_format, input_mol))
            print('debug3 --', input_mol)
            self.rd_mol = Chem.SDMolSupplier(input_mol)[0]
        elif mol_format == 'rdmol':
            self.rd_mol = input_mol
        else:
    ...
    def get_pdbqt(self, lig_pdbqt=None):
        ...
        # print("debug4 --", self.rd_mol)
        preparator.prepare(self.rd_mol)
...
class VinaDock(object): 
    ...
    def dock(self, score_func='vina', seed=0, mode='dock', exhaustiveness=8, save_pose=False):  # seed=0 mean random seed
...
class VinaDockingTask(BaseDockingTask):
    ...
    def __init__(self, protein_path, ligand_rdmol, tmp_dir='./tmp', use_uff=True, center=None,
                 size_factor=1., buffer=5.0, custom_size=None):
        ...
        if size_factor is None:
            if custom_size is None:
                ...
                self.size_x = custom_size
                self.size_y = custom_size
                self.size_z = custom_size
        else:
    ...
    def run(self, mode='dock', exhaustiveness=8):
        ...
        lig = PrepLig(self.ligand_rdmol, 'rdmol')
