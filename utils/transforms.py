...
class FeaturizeFragAtom(object):
    ...
    def __call__(self, data):
        element_list = data.frag_element
        hybridization_list = data.frag_hybridization
        aromatic_list = [v[AROMATIC_FEAT_MAP_IDX] for v in data.frag_atom_feature]

        x = [get_index(e, h, a, self.mode) for e, h, a in zip(element_list, hybridization_list, aromatic_list)]
        x = torch.tensor(x)
        data.frag_atom_feature_full = x
        return data
