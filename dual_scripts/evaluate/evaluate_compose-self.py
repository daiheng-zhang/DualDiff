...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(args.result_path, f'{idx1}/{idx2}/eval_all_self.pkl')):
        exit()
    ...
    protein_path = glob(f"data/processed/dock/ligand_protein_dataset_v2/{smiles1}/*/protein_clean.pdb")[0]
    anchor_ligand_dict = parse_sdf_file_mol(glob(f"data/processed/dock/ligand_protein_dataset_v2/{smiles1}/*/ligand.sdf")[0])

    
    testset_results = eval_single_datapoint(ligand_rdmol_list, protein_path, args, center=anchor_ligand_dict['center_of_mass'])

    if args.result_path:
        with open(os.path.join(args.result_path, f'{idx1}/{idx2}/eval_all_self.pkl'), 'wb') as f:
